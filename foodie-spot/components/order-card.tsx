...
const statusIcon: Record<Order['status'], React.ReactNode> = {
    ...
    'ready':      <CheckCircle size={16} color="#FF9800" />,
    'picked_up':  <Navigation size={16} color="#8B5CF6" />,
    ...
    return (
        <TouchableOpacity style={styles.card} onPress={onPress} disabled={!onPress}>
            ...
            <Text style={styles.items} numberOfLines={1}>
                {order.items?.map(item => item.dish?.name ?? item.name ?? '').join(', ') ?? ''}
            </Text>
...
const styles = StyleSheet.create({
    ...
    restaurant: { fontSize: 16, fontWeight: '700', flex: 1 },
    status: {
        flexDirection: 'row',
        alignItems: 'center',
        gap: 6,
    ...
    items: { color: '#666', marginBottom: 10 },
    footer: {
        flexDirection: 'row',
        justifyContent: 'space-between',
        alignItems: 'center',
